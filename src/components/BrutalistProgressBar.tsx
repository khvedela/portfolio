...
const BrutalistProgressBar = () => {
  ...
  return (
    <>
      ...
      {/* Corner progress indicator - brutalist style - DESKTOP ONLY */}
      <motion.div
        className="no-print hidden lg:block fixed bottom-6 left-6 z-50 bg-foreground text-background border-3 border-foreground font-mono font-bold select-none"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ delay: 0.5 }}
      >
        {/* Progress percentage */}
        <div className="px-4 py-3 text-lg leading-none flex items-center gap-2">
          <motion.div
        ...
        {/* Visual bar segments */}
        <div className="flex gap-px bg-background p-2 border-t-3 border-foreground">
          {[...Array(10)].map((_, i) => (
            <motion.div
              key={i}
              className="w-3 h-8 bg-muted"
              animate={{
                backgroundColor:
                  scrollPercentage >= (i + 1) * 10
                    ? i % 2 === 0
                      ? "hsl(var(--primary))"
                      : "hsl(var(--accent))"
                    : "hsl(var(--muted))",
        ...
        {/* Status text */}
        <div className="px-4 py-2 text-xs border-t-3 border-foreground uppercase tracking-wider">
          {scrollPercentage === 0 && "START"}
          {scrollPercentage > 0 && scrollPercentage < 100 && "SCROLLING"}
          {scrollPercentage === 100 && "END"}
        </div>
      </motion.div>
